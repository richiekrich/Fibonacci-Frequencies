...
def display_gif():
    ...
    label = AnimatedGIFLabel(root, gif_path)
    label.pack()
    root.mainloop()
...
#BPM Slider
bpm_scale = tk.Scale(root, from_=30, to=500, orient='horizontal', label='BPM')  # Extended range down to 30 BPM
...
# Play button
play_button = tk.Button(root, text='Play', command=start_playing)  # Link to the full arpeggio function
play_button.pack()
...
# Add the animated GIF label to the Tkinter window
gif_path = r'C:\Scripts\Fib.gif'  # Replace with your GIF's actual path
label = AnimatedGIFLabel(root, gif_path)
label.pack()
